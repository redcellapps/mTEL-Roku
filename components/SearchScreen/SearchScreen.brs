...
sub setColors()
    m.searchPalette = CreateObject("roSGNode", "RSGPalette")
    m.searchPalette.colors = {
...
function searchScreen()
    m.searchDialog = m.top.findNode("dialog")
    m.searchDialog.buttons = [m.langs[m.lng][30], m.langs[m.lng][19]]
    m.searchDialog.visible = false
    m.searchDialog.title = m.langs[m.lng][30]' "PRETRAGA..."
    m.searchDialog.message = [m.langs[m.lng][25]]
    m.searchDialog.palette = m.searchPalette
    m.searchDialog.visible = true
    m.searchDialog.setFocus(true)
    m.searchDialog.observeField("buttonSelected", "searchDialogAction")
end function

sub searchDialogAction()
    if m.searchDialog.buttonSelected = 0
        searchString = m.searchDialog.text
        searchContent(searchString)
        m.searchDialog.visible = false
        m.top.setFocus(true)
'    showSearchErr()
    else if m.searchDialog.buttonSelected = 1
        m.searchDialog.visible = false
        m.topMenu.setFocus(true)
    end if
end sub

sub searchContent(searchString)
    m.allContent = m.LoginScreen.contents
    m.searchResult = CreateObject("roSGNode", "ContentNode")
    for each channel in m.allContent[0][1]
        where = LCase(channel[0])
        what = LCase(searchString)
        if InStr(where, what) > 0
            itm = GetItemData(channel)
            itemcontent = m.searchResult.createChild("ContentNode")
            itemcontent.setFields(itm)
        end if
    end for
    if m.searchResult.getChildCount() > 0
        m.channelMarkupGrid.content = m.searchResult
...
sub showSearchErr()
    ...
    m.searchErr.title = "No Results"
    m.searchErr.message = ["Please try again"]
...
sub onEpgBackButtonSelected()
    ...
    m.epgMarkupGrid.jumpToItem = now - 30
    m.focusedEpgItem = now - 30
    m.EpgMarkupGrid.setFocus(true)
...
sub onEpgFwdButtonSelected()
    now = m.focusedEpgItem
    m.epgMarkupGrid.jumpToItem = now + 35
    m.focusedEpgItem = now + 35
    m.EpgMarkupGrid.setFocus(true)
...
sub onEpgLiveButtonSelected()
    m.firstFocus = true
    now = m.epgNow
    m.epgMarkupGrid.jumpToItem = now
    m.focusedEpgItem = now
    m.EpgMarkupGrid.setFocus(true)
'    m.EpgFwdButton.setFocus(true)
end sub
...
sub onTopMenuItemSelected()
    if m.topMenu.itemSelected = 0
     if m.VODGrid <> invalid
     m.VODGrid.visible = false
     end if
     searchScreen()
...
'        end if
'        getRadio()
    else if m.topMenu.itemSelected = 1
    if m.channelMarkupGrid <> invalid
        m.channelMarkupGrid.visible = false
        end if
    searchMovies()
'    else
...
'        dateObj = CreateObject("roDateTime")
'        tzo = dateObj.GetTimeZoneOffset()
'        if tzo < (-60) then tzo = tzo + 1440
'        offset = CINT(((now/60) - (showStart/60)) + tzo) + 60
'    else
'        offset = 0
''        showID = 0
'    end if
'    if offset <> invalid and offset > ((showEnd - showStart))/60
...
function getCurrentTimeSeconds()
    ...
    if timezoneOffset < 0 then timezoneOffset = timezoneOffset + 1440
    currentTime = dateObj.AsSeconds() - (timeZoneOffset * 60)
    return currentTime
end function
...
'                        end if
                        ...
                        if m.topMenu.hasFocus() = true
                            if m.topMenu.itemFocused = 0
                        searchScreen()
                            else if m.topMenu.itemFocused = 1
                            searchMovies()
                            end if
                            end if
