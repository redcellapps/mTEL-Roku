...
sub Init()
    ...
    m.player.enableDecoderStats = true
    m.player.observeField("state", "onPlayerStateChanged")
    m.player.observeField("decoderStats", "onDecoderStatsChanged")
...
function getEpg(chid as String)
    ...
    url = m.epgURL + chid + "/"
    ? "DEBUG - GET EPG FOR CHANNEL ID:";m.chid; "= "; url
...
'function onDecoderStatsChanged()
'    ? "DEBUG - PLAYER DECODER STATS = "; m.player.decoderStats
'    ? "DEBUG - VIDEO FORMAT: ";m.player.videoFormat
'    ? "DEBUG - STREAM INFO: ";m.player.streamInfo
'    ? "DEBUG - STREAM SEGMENT: ";m.player.streamingSegment
'    ? "DEBUG - DOWNLOADED SEGMENT: ";m.player.downloadedSegment
'    ? "DEBUG - SUDIO FORMAT: ";m.player.audioFormat
'    ? "DEBUG - CDN SWITCH: ";m.player.cdnSwitch
'end function

function onPlayerStateChanged()
    if m.player.state = "error"
    ? "DEBUG - PLAYER ERROR = "; m.player.errorMsg; m.player.errorCode
    end if
